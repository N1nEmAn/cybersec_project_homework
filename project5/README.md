...
基于10次独立测试的结果（最新测试时间：2025年7月13日）：

| 操作 | 平均时间 | 标准差 | 吞吐量 | 算法正确性 |
|------|----------|--------|--------|------------|
| 密钥生成 | 21.1ms | ±1.0ms | 47.4 ops/sec | ✅ 通过 |
| 数字签名 | 21.5ms | ±1.9ms | 46.5 ops/sec | ✅ 通过 |
| 签名验证 | 41.2ms | ±1.1ms | 24.3 ops/sec | ✅ 通过 |

### 性能特点分析

- **密钥生成**: 稳定性优秀(标准差1.0ms)，适合批量密钥生成
- **数字签名**: 性能最优(21.5ms)，随机数生成是主要开销
- **签名验证**: 耗时最长(41.2ms)，需要两次点乘运算
- **算法正确性**: 通过了包括中文字符、二进制数据等7种消息类型测试（100%成功率）
...
**验证正确性证明**:
若签名有效，则 [s]G + [t]P = [s]G + [(r+s)]P = [s]G + [r][d]G + [s][d]G = [s + rd + sd]G = [k]G，
因此 x₁ 坐标相同，验证等式 R = r 成立。
...
其中 a = -3, b 为曲线参数，p 为大素数

**签名过程数学推导**:
1. 计算消息哈希 e = H(m)
2. 生成随机数 k ∈ [1, n-1]
3. 计算椭圆曲线点 (x₁, y₁) = k·G，其中 G 为基点
4. 计算签名第一部分 r = (e + x₁) mod n
5. 计算签名第二部分 s = d⁻¹(k - r·d) mod n
6. 输出签名 (r, s)

**验证过程数学推导**:
1. 计算消息哈希 e = H(m)
2. 计算辅助值 t = (r + s) mod n
3. 计算椭圆曲线点 (x₁, y₁) = s·G + t·P，其中 P 为公钥
4. 验证条件 R = (e + x₁) mod n
5. 验证成功当且仅当 R = r

**安全性数学基础**:
- 基于椭圆曲线离散对数问题(ECDLP)的困难性
- 256位SM2等效于3072位RSA的安全强度
- Pollard's rho攻击复杂度为 O(√n) ≈ 2¹²⁸ 次运算
...
**统计性能 (10次测试平均)**:
- 密钥生成: 21.1ms ± 1.0ms (47.4 ops/sec)
- 数字签名: 21.5ms ± 1.9ms (46.5 ops/sec)
- 签名验证: 41.2ms ± 1.1ms (24.3 ops/sec)
