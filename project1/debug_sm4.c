#include <stdio.h>
#include <stdint.h>
#include <string.h>

/* 简化的SM4参考实现用于调试 */

/* SM4 S-Box - 确认正确 */
const uint8_t sbox[256] = {
    
};

const uint32_t FK[4] = {0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC};
const uint32_t CK[32] = {
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B222B, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
};

uint32_t ROTL(uint32_t x, int n) {
    return (x << n) | (x >> (32 - n));
}

uint32_t tau(uint32_t A) {
    return (uint32_t)sbox[(A >> 24) & 0xFF] << 24 |
           (uint32_t)sbox[(A >> 16) & 0xFF] << 16 |
           (uint32_t)sbox[(A >> 8) & 0xFF] << 8 |
           (uint32_t)sbox[A & 0xFF];
}

uint32_t L(uint32_t B) {
    return B ^ ROTL(B, 2) ^ ROTL(B, 10) ^ ROTL(B, 18) ^ ROTL(B, 24);
}

uint32_t L_prime(uint32_t B) {
    return B ^ ROTL(B, 13) ^ ROTL(B, 23);
}

void SM4_KeySchedule(const uint8_t MK[16], uint32_t rk[32]) {
    uint32_t K[4];
    for (int i = 0; i < 4; i++) {
        K[i] = (uint32_t)MK[4*i] << 24 | (uint32_t)MK[4*i+1] << 16 | 
               (uint32_t)MK[4*i+2] << 8 | (uint32_t)MK[4*i+3];
        K[i] ^= FK[i];
    }
    
    for (int i = 0; i < 32; i++) {
        rk[i] = K[0] ^ L_prime(tau(K[1] ^ K[2] ^ K[3] ^ CK[i]));
        K[0] = K[1]; K[1] = K[2]; K[2] = K[3]; K[3] = rk[i];
    }
}

void SM4_Encrypt(const uint8_t plaintext[16], const uint32_t rk[32], uint8_t ciphertext[16]) {
    uint32_t X[4];
    for (int i = 0; i < 4; i++) {
        X[i] = (uint32_t)plaintext[4*i] << 24 | (uint32_t)plaintext[4*i+1] << 16 |
               (uint32_t)plaintext[4*i+2] << 8 | (uint32_t)plaintext[4*i+3];
    }
    
    for (int i = 0; i < 32; i++) {
        uint32_t tmp = X[1] ^ X[2] ^ X[3] ^ rk[i];
        tmp = L(tau(tmp));
        tmp ^= X[0];
        X[0] = X[1]; X[1] = X[2]; X[2] = X[3]; X[3] = tmp;
    }
    
    // 最终变换：(X35, X34, X33, X32)
    for (int i = 0; i < 4; i++) {
        uint32_t Y = X[3-i];
        ciphertext[4*i] = (Y >> 24) & 0xFF;
        ciphertext[4*i+1] = (Y >> 16) & 0xFF;
        ciphertext[4*i+2] = (Y >> 8) & 0xFF;
        ciphertext[4*i+3] = Y & 0xFF;
    }
}

int main() {
    // 测试向量1
    uint8_t key1[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 
                        0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
    uint8_t pt1[16] =  {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 
                        0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
    uint8_t ct1[16];
    uint32_t rk[32];
    
    SM4_KeySchedule(key1, rk);
    SM4_Encrypt(pt1, rk, ct1);
    
    printf("Test Vector 1:\n");
    printf("Expected: 681EDF34D206965E86B3E94F536E4246\n");
    printf("Computed: ");
    for (int i = 0; i < 16; i++) {
        printf("%02X", ct1[i]);
    }
    printf("\n\n");
    
    return 0;
}
